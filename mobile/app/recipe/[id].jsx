...
const RecipeDetailScreen = () => {
    ...
    useEffect(() => {
        const checkIfSaved = async () => {
            try {
                ...
                const fav = favorites.find((f) => f.recipeId === parseInt(recipeId, 10));

                setIsSaved(!!fav);
                setRatingId(fav?.ratingId ?? null);

                // console.log(ratingId, recipeId);
            } catch (error) {
    ...
    const handleRateRecipe = async (newRatingId) => {
        setIsSaving(true);
        try {
            if (!isSaved) {
                ...
                const response = await fetch(url, {
                    ...
                    body: JSON.stringify({
                        ...
                        cookTime: recipe.cookTime,
                        servings: recipe.servings,
                        ratingId: parseInt(newRatingId, 10),
                    }),
                });
                ...
                const url = `${API_URL}/favorites/${userId}/${recipeId}/rating`;
                ...
                console.log('PUT rating →', url);

                if (!rateRes.ok) {
                    const body = await rateRes.text();
                    throw new Error(`HTTP ${rateRes.status} – ${body.slice(0, 300)}`);
                }
                setRatingId(newRatingId);
            }
        } catch (error) {
        ...
        <View style={recipeDetailStyles.container}>
            <ScrollView>
                ...
                <View style={recipeDetailStyles.contentSection}>
                    ...
                    <View style={recipeDetailStyles.ratingScrollContent}>
                        <TouchableOpacity
                            onPress={() => handleRateRecipe(1)}
                            style={[
                                recipeDetailStyles.ratingButton,
                                ratingId === 1 && { borderColor: COLORS.primary, borderWidth: 2 },
                            ]}
                        >
                            <Image
                                ...
                                transition={300}
                            />
                            <Text style={recipeDetailStyles.categoryText}>Nopeee</Text>
                        </TouchableOpacity>

                        <TouchableOpacity
                            onPress={() => handleRateRecipe(2)}
                            style={[
                                recipeDetailStyles.ratingButton,
                                ratingId === 2 && { borderColor: COLORS.primary, borderWidth: 2 },
                            ]}
                        >
                            <Image
                                ...
                                transition={300}
                            />
                            <Text style={recipeDetailStyles.categoryText}>Meh</Text>
                        </TouchableOpacity>

                        <TouchableOpacity
                            onPress={() => handleRateRecipe(3)}
                            style={[
                                recipeDetailStyles.ratingButton,
                                ratingId === 3 && { borderColor: COLORS.primary, borderWidth: 2 },
                            ]}
                        >
                            <Image
                                ...
                                transition={300}
                            />
                            <Text style={recipeDetailStyles.categoryText}>Awesome</Text>
                        </TouchableOpacity>

                        <TouchableOpacity
                            onPress={() => handleRateRecipe(4)}
                            style={[
                                recipeDetailStyles.ratingButton,
                                ratingId === 4 && { borderColor: COLORS.primary, borderWidth: 2 },
                            ]}
                        >
                            <Image
                                ...
                                transition={300}
                            />
                            <Text style={recipeDetailStyles.categoryText}>TBD</Text>
                        </TouchableOpacity>
                    </View>
